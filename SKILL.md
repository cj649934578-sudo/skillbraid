...
## Always-On Route Guidance

When a request involves skill usage, task planning, code changes, testing, GitHub, documents, or design, prefer using SkillBraid to decide the route before proceeding.

当用户请求涉及 skill 使用、任务规划、代码修改、测试、GitHub、文档或设计时，优先使用 SkillBraid 判断链路，并在回答末尾输出：
...
This rule applies when SkillBraid is active or explicitly invoked. If a higher-priority user instruction conflicts with it, follow the user's current-turn instruction and briefly explain the conflict.

## Storage Locations

Use these locations unless the user or project instructions specify another path:

- Global preferences: `%USERPROFILE%\.codex\skill-routes\global-routes.json`
- Global usage summary: `%USERPROFILE%\.codex\skill-routes\global-usage.json`
- Project routes: `.codex\skill-routes\project-routes.json`
- Project usage records: `.codex\skill-routes\project-usage.json`

Before creating any of these files, explain what will be written. Route rule changes require explicit user confirmation. Usage records can be appended after initialization has been confirmed for the project, but each record must be concise.
...
8. Ask about one recurring user scenario at a time.
9. For each confirmed scenario, propose a candidate route with trigger conditions, skip conditions, chain order, reasons, and risks.
10. Write route rules only after the user confirms the exact route and storage layer.

## Daily Routing Mode

Use daily routing mode when the user asks for work that may match an existing route.

1. Read project routes first if they exist.
2. Read global preferences second if they exist.
3. Apply this priority:
   - current-turn explicit user instruction
   - project route rules
   - global user preferences
   - session judgment
4. If a route matches, follow it and keep moving.
5. If the request is a new scenario, propose one candidate route and wait for confirmation before treating it as a standing route.
6. If multiple routes match, explain the conflict briefly and prefer the project route unless the user says otherwise.
7. If a needed skill is missing, installed but hidden, invalid, or duplicated, state the gap and help the user search for or install a suitable skill.
8. If the user explicitly names a skill, respect that choice and explain how it relates to the route.
...
Track concise usage signals after routed work when route storage has been initialized.
...
- `references/route-rules.schema.json` defines route rule structure.
- `references/route-rules.example.json` shows a confirmed route rule.
- `references/usage-record.schema.json` defines concise usage record structure.
- `references/usage-record.example.json` shows an optimization record.
